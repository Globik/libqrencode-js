...
public:
	...
	protected:
	void Execute() override{sum=arg0+arg1;}
	void OnOK() override{
	Napi::Env env=Env();
	Callback().MakeCallback(Receiver().Value(),{env.Null(),Napi::Number::New(env,sum)});
	}
	void OnError(const Napi::Error& e) override{
	Napi::Env env=Env();
		Callback().MakeCallback(Receiver().Value(),{e.Value(),env.Undefined()});
...
Napi::Env env=info.Env();
	...
	Napi::TypeError::New(env,"invalid arg types").ThrowAsJavaScriptException();
		return;
	}
	Napi::Function cb=info[2].As<Napi::Function>();
	if(info.Length() !=3){
	(new ErrorAsyncWorker(cb,Napi::TypeError::New(env,"inv arg types")))->Queue();
	}else if(!info[0].IsNumber() || !info[1].IsNumber()){
	(new ErrorAsyncWorker(cb,Napi::TypeError::New(env,"inv argi typ")))->Queue();
	}else{
...
exports.Set(Napi::String::New(env,"add"),Napi::Function::New(env,SumAsyncCallback));
	return exports;
}
